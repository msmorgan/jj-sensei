...
one-command answer before any manual choreography is considered.
...
*mutable* ancestor that last modified those lines. A hunk whose destination is
ambiguous stays behind in the source, so the command is safe to run and then
inspect. If everything is absorbed and the source has no description, the
...
Reach past `absorb` only when the destination is not the commit that last
touched those lines — a new file, or a hunk that belongs somewhere `absorb`
would not choose.
...
Either `--from` or `--into` defaults to `@` when omitted. Always give one of
`-m` or `-u`: when both source and destination have nonempty descriptions and
the source is emptied, jj otherwise opens an editor to combine them. `-m` sets
the destination's description; `-u`/`--use-destination-message` keeps the
destination's existing description and discards the source's, which is what is
usually wanted when squashing a fix into a commit that is already well
described.
...
- `-m` describes the selected half only. The remainder keeps `REV`'s original
  description verbatim, so it is now mislabelled. A final `jj --no-pager
...
- Address the remainder as `REV+` — the child of the change ID that stayed
  with the selected half. Do not parse the split's output for its ID.
...
| Form | Matches |
|---|---|
| `src/parser.rs` | cwd-relative path prefix — a file, or a directory recursively |
| `file:"src/parser.rs"` | that cwd-relative path exactly |
...
`jj diff '~"Foo Bar"'` needs both shell and inner quotes. Glob characters are
not meta characters for that rule, but shell quoting is still required.
...
Prefer `-r` with an explicit revset, and preview it before mutating:
...
the mutable anonymous heads that no workspace and no bookmark holds:
