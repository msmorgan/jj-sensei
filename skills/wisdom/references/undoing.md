...
| One file put back the way an earlier revision had it, in the working copy | `jj --no-pager restore --from REV PATH` | Pulls that revision's version **into** `@`; `--into` defaults to `@`. Always pass `PATH`: with no fileset, `--from` copies `REV`'s **entire tree** into the destination. |
| A described change rolled back to an earlier version of **itself** (undo part of an amend) | `jj --no-pager restore --from SNAPSHOT --into CHANGE PATH` | `SNAPSHOT` is a commit ID from `CHANGE`'s own evolog; `--into CHANGE` is the correct form here, not a trap — it rewrites that change's tree for those paths and descendants rebase. Never restore the whole tree elsewhere and rebase `CHANGE` onto it. See [Restore a change to one of its own snapshots](using-evolog.md#restore-a-change-to-one-of-its-own-snapshots). |
